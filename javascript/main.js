...
theme1 = new Image()
theme1.src = 'img/og-theme.webp'
theme2 = new Image()
theme2.src = 'img/og-theme-2.webp'
frame = 0;
degree = Math.PI/180
SFX_scoreflappy.src = 'audio/sfx_point.wav'
...
//current scoreflappy, top scoreflappy, tracker
scoreflappy = {
    current: 0,
    ...
    //display the scoreflappy
    render: function() {
        if (gameState.current == gameState.play ||
            gameState.current == gameState.gameOver) {
            //change current scoreflappy number value to string value and access each place value
            ...
            //if current scoreflappy has thousands place value: the game is over
            if (this.current >= 1000) {
                gameState.current = gameState.gameOver
            
            //if current scoreflappy has ones, tens, and hundreds place value only
            ...
            //if current scoreflappy has ones and tens place value only
            ...
            //if current scoreflappy has ones place value only
            } else {
